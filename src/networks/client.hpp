...
public:
	...
	forwarder() = delete;
	forwarder(asio::io_context &io_context, ttp::task_group_pool &task_pool, size_t task_count_limit, bool use_thread_pool, std::shared_ptr<KCP::KCP> input_kcp, process_data_t callback_func) :
...
class tcp_to_forwarder
{
	...
	void udp_client_incoming_to_tcp_with_thread_pool(std::shared_ptr<KCP::KCP> kcp_ptr, std::unique_ptr<uint8_t[]> data, size_t data_size, udp::endpoint peer, asio::ip::port_type local_port_number);
	...
	int kcp_sender_with_pool(KCP::KCP *kcp_ptr, tcp_session *session, const char *buf, int len, void *user);
...
class udp_to_forwarder
{
	...
	void udp_client_incoming_to_udp_with_thread_pool(std::shared_ptr<KCP::KCP> kcp_ptr, std::unique_ptr<uint8_t[]> data, size_t data_size, udp::endpoint peer, asio::ip::port_type local_port_number);
	...
	int kcp_sender_with_pool(const char *buf, int len, void *user);
	udp::endpoint get_remote_address();
